...
def get_circles(image):
  ...
  circles = cv2.HoughCircles(mask,cv2.HOUGH_GRADIENT,dp=1,minDist=20,param1=100,param2=8,minRadius=1, maxRadius=15) 
...
def three_circles(circles):
  ...
  #circles = np.array([circle1, circle2, circle3], dtype=dtype)
  circles = [circle1, circle2, circle3]
  #sorted_circles = np.sort(circles, order='y')
...
def four_circles(circles):
  ...
  #dtype = np.dtype([('x',"int"), ('y',"int")])
  #circles = np.array([circle1, circle2, circle3, circle4], dtype=dtype)
...
def get_joints(image):
  ...
  if num_circles==2:
    ...
    circle1, circle2, circle3, circle4 = None 
...
def identify_joints(joints1, num_circles1, joints2, num_circles2):
  if num_circles1 == num_circles2 and num_circles1 == 4:
    ...
    # joints identified, converted to be with respect to fixed frame and in meters
    redPos, greenPos, bluePos, yellowPos = joint_wrt_base(compare_joints(circle1, circle2, circle3, circle4))
    ...
    redPos, greenPos, bluePos, yellowPos = joint_wrt_base(joint_pos_3d(redPos1, greenPos1, bluePos1, yellowPos1,redPos2, greenPos2, bluePos2, yellowPos2))
